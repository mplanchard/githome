...
var ShellWindow = class ShellWindow {
    ...
    bind_window_events() {
        this.ext.window_signals.get_or(this.entity, () => new Array())
            .push(this.meta.connect('size-changed', () => { this.window_changed(); }), this.meta.connect('position-changed', () => { this.window_changed(); }), this.meta.connect('workspace-changed', () => { this.workspace_changed(); }), this.meta.connect('raised', () => { this.window_raised(); }));
    }
    bind_hint_events() {
        ...
        if (border) {
            ...
            if (stack_number !== null) {
                ...
                if (stack) {
                    ...
                    if (borderSize === 0 || this.grab === null) {
                        stack_tab_height = 0;
                    }
                    dimensions = [
                        x - borderSize,
...
function activate(default_pointer_position, win) {
    win.raise();
    win.unminimize();
